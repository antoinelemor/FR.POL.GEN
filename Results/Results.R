...
export_path <- "/Users/antoine/Documents/GitHub/FR.POL.GEN/FR.POL.GEN/Results/Graph"
...
ggplot(far_right_score_time_series, aes(x = date, y = far_right_score, color = detect_type_clean)) +
  ...
  theme_minimal() +
  labs(title = "Évolution du score de droite extrême des thématiques dans le temps",
       x = "Date",
       y = "Score d'Extrême Droite",
       color = "Thématique") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "right") +
...
ggplot(score_data_date, aes(x = reorder(intervenant_date, far_right_score), y = far_right_score, fill = intervenant)) +
  ...
  theme_minimal() +
  labs(title = "Scores de Droite Extrême par Premier(e) Ministre et Date",
...
ggplot(score_data_date, aes(x = date, y = far_right_score, group = intervenant)) +
...
ggsave(filename = "FRS_through_time.pdf", path=export_path, width = 10, height = 8, units = "in")
